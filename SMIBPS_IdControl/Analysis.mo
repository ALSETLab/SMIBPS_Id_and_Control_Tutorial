...
<td><p><a href=\"mailto:luigiv@kth.se\">luigiv@kth.se</a></p></td>
      ...
      model LinearModelInterface
        ...
        Modelica.Blocks.Sources.Constant y0Exp[ny](k=k)
          annotation (Placement(transformation(extent={{-10,-10},{10,10}},
              rotation=90,
              origin={40,-26})));
        Modelica.Blocks.Routing.DeMultiplex5 demultiplex2_5
          annotation (Placement(transformation(extent={{90,-10},{110,10}})));
        Modelica.Blocks.Interfaces.RealInput input1
          annotation (Placement(transformation(extent={{-180,40},{-140,80}})));
        Modelica.Blocks.Interfaces.RealInput input2
          annotation (Placement(transformation(extent={{-180,-80},{-140,-40}})));
        Modelica_LinearSystems2.Controller.StateSpace stateSpace(system=system)
          annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
      ...
      equation
        connect(y0Exp.y,addy. u2) annotation (Line(points={{40,-15},{40,-6},{
                50,-6}},color={0,0,127}));
        connect(addy.y,demultiplex2_5. u)
          annotation (Line(points={{73,0},{88,0}},           color={0,0,127}));
        connect(input1, multiplex2_1.u1[1]) annotation (Line(points={{-160,60},
                {-88,60},{-88,6},{-52,6}},
                                  color={0,0,127}));
        connect(input2, multiplex2_1.u2[1]) annotation (Line(points={{-160,
                -60},{-84,-60},{-84,-6},{-52,-6}},
                                    color={0,0,127}));
        connect(demultiplex2_5.y1[1], Vt) annotation (Line(points={{111,8},{130,8},{130,
                79},{150,79}},             color={0,0,127}));
        connect(demultiplex2_5.y2[1], P) annotation (Line(points={{111,4},{132,4},{132,
                41},{150,41}},             color={0,0,127}));
        connect(w, demultiplex2_5.y4[1]) annotation (Line(points={{150,-39},{130,-39},
                {130,-4},{111,-4}},          color={0,0,127}));
        connect(delta, demultiplex2_5.y5[1]) annotation (Line(points={{150,-81},{120,-81},
                {120,-8},{111,-8}},                color={0,0,127}));
        connect(demultiplex2_5.y3[1], Q) annotation (Line(points={{111,0},{130,0},{130,
                1},{150,1}},             color={0,0,127}));
        connect(stateSpace.u, multiplex2_1.y)
          annotation (Line(points={{-12,0},{-29,0}}, color={0,0,127}));
        connect(stateSpace.y, addy.u1) annotation (Line(points={{11,0},{30,0},
                {30,6},{50,6}}, color={0,0,127}));
        annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics={
              Rectangle(
                ...
                fillPattern=FillPattern.Solid),
              Text(
                extent={{124,-32},{-126,28}},
                lineColor={28,108,200},
                textString="Linearized Model"),
              Text(
                extent={{140,-138},{-140,-80}},
                lineColor={238,46,47},
              ...
              Text(
                ...
                fillColor={255,0,0},
                textString="y")}));
      end LinearModelInterface;

      model TestLinModelInterface
        extends OutputsInterface;
        LinearModelInterface linearModelInterface annotation (Placement(transformation(extent={{-40,-58},{80,60}})));
        Modelica.Blocks.Sources.Step stepEfd(height=0.05, startTime=1)
          annotation (Placement(transformation(extent={{-134,16},{-114,36}})));
        Modelica.Blocks.Sources.Step stepPm(height=0, startTime=1)
          annotation (Placement(transformation(extent={{-134,-34},{-114,-14}})));
      initial algorithm
      equation
        connect(linearModelInterface.Vt, Vt) annotation (Line(points={{84.2857,
                34.2929},{113.143,34.2929},{113.143,79},{150,79}},
                                                          color={0,0,127}));
        connect(linearModelInterface.P, P) annotation (Line(points={{84.2857,
                18.2786},{115.143,18.2786},{115.143,41},{150,41}},
                                                          color={0,0,127}));
        connect(linearModelInterface.Q, Q) annotation (Line(points={{84.2857,
                1.42143},{115.143,1.42143},{115.143,1},{150,1}},
                                                        color={0,0,127}));
        connect(linearModelInterface.w, w) annotation (Line(points={{84.2857,
                -15.4357},{114.143,-15.4357},{114.143,-39},{150,-39}},
                                                             color={0,0,127}));
        connect(linearModelInterface.delta, delta) annotation (Line(points={{84.2857,
                -33.1357},{102.143,-33.1357},{102.143,-81},{150,-81}},
                                                             color={0,0,127}));
        connect(linearModelInterface.input1, stepEfd.y) annotation (Line(points={{
                -48.5714,26.2857},{-81.2857,26.2857},{-81.2857,26},{-113,26}},
                                                                      color={0,0,127}));
        connect(linearModelInterface.input2, stepPm.y) annotation (Line(points={{
                -48.5714,-24.2857},{-80.2857,-24.2857},{-80.2857,-24},{-113,
                -24}},                                                    color={0,0,127}));
        annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
              coordinateSystem(preserveAspectRatio=false)));
      end TestLinModelInterface;

      function GenerateAndSaveLinearModel
        "Generate a linear state space system from a (nonlinear) Modelica model and save it in the current directory"
        extends Modelica.Icons.Function;

        import Modelica_LinearSystems2;
        import Modelica_LinearSystems2.StateSpace;

        input String modelName="SMIB_PSControl.AnalysisVariants.LinearAnalysis.Interfaces.SMIB_GEN_wInput";
...
This example demonstrates how to generate a linear state space system from a (nonlinear) Modelica model.
      ...
      end GenerateAndSaveLinearModel;
    end Interfaces;
